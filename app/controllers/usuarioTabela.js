...
module.exports = function (app) {
	var controller = {};
	var Proxy = app.models.usuarioTabela;
	
	controller.listar = function(req, res) {
		Proxy.find().populate('usuario').populate('tabela').deepPopulate('tabela.empresa').sort('usuario').exec()
			.then(function(dados) {
				res.json(dados);
			},
			function (erro) {
	...
	controller.listarTabelasPorUsuario = function(req, res) {
		pUsuario = req.params.idusuario;
		Proxy.find({
				'usuario': pUsuario
			}).populate('tabela').deepPopulate('tabela.empresa').sort('tabela').exec()
			.then(function(dados) {
				res.json(dados);
			},
			function (erro) {
